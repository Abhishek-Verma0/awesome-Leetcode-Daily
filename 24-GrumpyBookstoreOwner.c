...
int maxSatisfied(int* customers, int n, int* grumpy, int gz, int minutes) {
    int satisfied=0;
    for(register int i=0; i<n; i++)
        satisfied+=(i<minutes|| grumpy[i]==0)?customers[i]:0;
    int max_satisfied=satisfied;
    for (register int i=minutes; i<n; i++){
            ...
            max_satisfied =(max_satisfied<satisfied)?satisfied:max_satisfied;
    }
